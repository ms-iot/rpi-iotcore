...
Abstract:
    ...
    This module contains the interrupt service routine for Pi mini Uart
    serial driver.
...
// from BCM2836 data sheet when DLAB=0

// bit 0 - Tx interrupt (interrupt line is asserted whenever the receive FIFO holds at least 1 byte)
// bit 1 - Rx interrupt (interrupt line is asserted whenever the transmit FIFO is empty)
// bits 2:7 - Reserved, write zero, read as don’t care

// from BCM2836 Errata and Phil

// ************* Bits 1:0 are swapped. ***********************
// bit 0 - receive interrupt (RX FIFO not empty)
// bit 1 - transmit interrupt (TX FIFO empty)

// Bits 3:2 in errata - may have has to be set.
// other notes - Phil and community. Bits 3:2 maybe unsure if need to be set
...

{
    ...
    SHORT sReadFifoLvl=0;
    SHORT sTransmitFifoLvl=0;
    UCHAR tempLSR;
    ...
    TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "++SerialISR(msg=%Xh) c=%lu\r\n",MessageID,ulIsrCallCount);
    ...
    else if (Extension->DeviceIsOpened==FALSE /* || (Extension->PowerState != PowerDeviceD0)*/) 
    {
        //
        ...
        //
        TraceEvents(TRACE_LEVEL_WARNING, DBG_INTERRUPT, "SerialISR [%lu] - miniUart interrupt with device Closed\r\n",ulIsrInnerLoopCnt);
        ulIsrInnerLoopCnt=0;
        ...
        do 
        {
            ...
            switch (InterruptIdReg) 
            {
                ...
                case SERIAL_IIR_THR:    // 2, transmit
                ...
                case SERIAL_IIR_RLS: // 6, this is an error condition on miniuart
                case SERIAL_IIR_MS: // 0
                default:
                TraceEvents(TRACE_LEVEL_ERROR, DBG_INTERRUPT, "SerialISR(no) [%lu] - invalid IIReg=%02Xh\r\n",ulIsrInnerLoopCnt,InterruptIdReg);
                ASSERT(FALSE);
                break;
            };
    ...
    {
        ulIsrInnerLoopCnt=0;
        TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart interrupt. IIReg=%02Xh\r\n",ulIsrInnerLoopCnt,InterruptIdReg);
        ServicedAnInterrupt = TRUE;
        do {
            ...
            switch (InterruptIdReg) 
            {
                ...
                case SERIAL_IIR_RDA: // 4, 10b : Receiver holds valid byte - was original
                {
                    TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - RCV miniUart interrupt, IIR=%02Xh\r\n",ulIsrInnerLoopCnt,InterruptIdReg);
                    //
                    ...
                    // Pi mini Uart can report how many chars are in read fifo: bits 16-19 Rx FIFO level

                    sReadFifoLvl=(SHORT)((READ_EXTRA_STATUS(Extension->Controller) & 0x000F0000)>>16);
                    ...
                    do {
                        ...
                        TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR [%lu] - recvd %02Xh byte. Rx FIFO lvl=%u\r\n",
                                            ulIsrInnerLoopCnt,ReceivedChar, sReadFifoLvl); 
...
ReceiveDoLineStatus:    ;
                        TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu]- miniUart RCV interrupt, do line status\r\n",ulIsrInnerLoopCnt);
                        //
                        ...
                        if( !((tempLSR = SerialProcessLSR(Extension)) & SERIAL_LSR_DR) ) // This bit 0 1=the receiver FIFO holds at least 1 symbol. 
                            // extended status register - bit 0 1=Symbol available, bit 2 - 1=receiver idle, bits 16-19 - receiver FIFO count
                        { 
                            ...
                            // loop.
                            //
                            TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart RCV interrupt, no more chars\r\n",ulIsrInnerLoopCnt);
                            break;
                        ...
                        if ((tempLSR & ~(SERIAL_LSR_THRE | SERIAL_LSR_TEMT |
                           ...
                           //
                           TraceEvents(TRACE_LEVEL_WARNING, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart RCV interrupt, error LSR\r\n",ulIsrInnerLoopCnt);
                           break;
                        }

                        sReadFifoLvl--;

                    } WHILE (sReadFifoLvl>0);

                    break;

                }

                case SERIAL_IIR_THR: // 2, 01b : Transmit holding register empty - was original
                {
//doTrasmitStuff:;
                    TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu]- miniUart TX interrupt, IIR=%02Xh\r\n",ulIsrInnerLoopCnt,InterruptIdReg);
                    ...
                    if (Extension->WriteLength ||
                        ...
                        TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu]- miniUart TX interrupt, WrLen=%u XmtImmd=%d\r\n",ulIsrInnerLoopCnt,Extension->WriteLength,Extension->TransmitImmediate);
                        //
                        ...
                        if (Extension->SendXonChar &&
                            ...
                            if ((Extension->HandFlow.FlowReplace &
                                ...
                                // this character.
                                //
                                TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart TX interrupt, write Xon, SetRTS\r\n",ulIsrInnerLoopCnt);
                                ...
                                TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart TX interrupt, write Xon\r\n",ulIsrInnerLoopCnt);
                            ...
                            if ((Extension->HandFlow.FlowReplace &
                                ...
                                // this character.
                                //
                                TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart TX interrupt, write Xoff,set RTS\r\n",ulIsrInnerLoopCnt);
                                SerialSetRTS(Extension->WdfInterrupt, Extension);
                                ...
                                TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart TX interrupt, write Xoff\r\n",ulIsrInnerLoopCnt);
                            ...
                            if (!(Extension->HandFlow.FlowReplace &
                                ...
                                if ((Extension->HandFlow.FlowReplace &
                                     SERIAL_RTS_MASK) ==
                                     SERIAL_TRANSMIT_TOGGLE) {
                                    TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart TX interrupt, queue start timer lower Rts\r\n",ulIsrInnerLoopCnt);
                                    SerialInsertQueueDpc(
                                        Extension->StartTimerLowerRTSDpc
                                        )?Extension->CountOfTryingToLowerRTS++:0;
                        ...
                        //
                            ...
                            if ((Extension->HandFlow.FlowReplace &
                                ...
                                // this character.
                                //
                                TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart TX interrupt, write 1 byte, set RTS\r\n",ulIsrInnerLoopCnt);
                                SerialSetRTS(Extension->WdfInterrupt, Extension);
                                ...
                                TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart TX interrupt, write 1 byte, no RTS\r\n",ulIsrInnerLoopCnt);
                                Extension->PerfStats.TransmittedCount++;
                                Extension->WmiPerfData.TransmittedCount++;
                                WRITE_TRANSMIT_HOLDING(Extension,
                                    Extension->Controller,
                            ...
                            if ((Extension->HandFlow.FlowReplace &
                                ...
                                // this character.
                                //
                                TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart TX interrupt, writeXmit %u bytes, set Rts\r\n",ulIsrInnerLoopCnt,amountToWrite);
                                SerialSetRTS(Extension->WdfInterrupt, Extension);
                                ...
                                SerialInsertQueueDpc(
                                    ...
                                    TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR(o) [%lu] - miniUart TX interrupt, writeXmit %u bytes \r\n",ulIsrInnerLoopCnt,amountToWrite);
                            ...
                            {
                                TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR [%lu] - write complete. Disable Tx intrpt\r\n",ulIsrInnerLoopCnt);
                                //
                                ...
                                // 
                                // tx interrupt is on and tx fifo is not empty. Yet nothing to write. Turn off tx interrupt now.
                                //
                                uchIerTemp=READ_INTERRUPT_ENABLE(Extension,Extension->Controller);
                                WRITE_INTERRUPT_ENABLE(Extension,Extension->Controller, (uchIerTemp & ~SERIAL_IER_THR));
                    ...
                    {
                        sTransmitFifoLvl=(SHORT)((READ_EXTRA_STATUS(Extension->Controller) & 0x0F000000)>>24); // bits 27-24 Tx FIFO level
                        TraceEvents(TRACE_LEVEL_WARNING, DBG_INTERRUPT, "SerialISR [%lu] - stuck Tx interrupt. writelen=%Xh, Tx FIFO lvl=%u\r\n",ulIsrInnerLoopCnt,Extension->WriteLength,sTransmitFifoLvl);
                        if(0==Extension->WriteLength)
                        {
                            uchIerTemp=READ_INTERRUPT_ENABLE(Extension,Extension->Controller);
                            WRITE_INTERRUPT_ENABLE(Extension,Extension->Controller, (uchIerTemp & ~SERIAL_IER_THR));
                ...
                default: // ubnormal case
                TraceEvents(TRACE_LEVEL_WARNING, DBG_INTERRUPT, "SerialISR [%lu] - unsupported IIR content\r\n",ulIsrInnerLoopCnt);
                break;

            }; // end SWITCH

            // if determined that miniUart is stuck with receive interrupt line (IIR will read C4 and Rx will read 0x00 byte)
            // allow ISR to return to avoid DPC watchdog timeout.
            // If this condition persists

            if(iReadCnt>8) // miniUart receive FIFO can hold max 8 bytes. If receiver got stuck, make note of it and quit the loop 
            {
                TraceEvents(TRACE_LEVEL_WARNING, DBG_INTERRUPT, "SerialISR [%lu] - receiver stuck with ReadInterrupt. Forced isr loop quit\r\n",ulIsrInnerLoopCnt);
                break;
            };
        ...
        TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "SerialISR() c=%lu out of loop. IIR=%02Xh\r\n",ulIsrCallCount,InterruptIdReg);

        // note: miniUart on Pi platform has bit 5 set if there is at least one(1) byte room in Tx FIFO
        //  but that does not mean Tx FIFO is empty.
        // despite original note below from 16C550 driver, the section below causes losing one byte on transmit in miniuart

        //
        // Besides catching the WINBOND and SMC chip problems this
        // will also cause transmission to restart in case of an xon
        // char being received.  Don't remove.
        //
/*
        if (SerialProcessLSR(Extension) & SERIAL_LSR_THRE) // check bit 5
        {
            ...
            if (!Extension->TXHolding &&
                (Extension->WriteLength ||
                 Extension->TransmitImmediate)) 
            {
                TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INTERRUPT, "SerialISR() c=%lu back to transmit stuff. WriteLen=%u\r\n",ulIsrCallCount,Extension->WriteLength);
                goto doTrasmitStuff;
...
*/
    ...
    TraceEvents(TRACE_LEVEL_ISROUTP, DBG_INTERRUPT, "--SerialISR()=%lu c=%lu\r\n",ServicedAnInterrupt,ulIsrCallCount);
    ulIsrCallCount+=1;
