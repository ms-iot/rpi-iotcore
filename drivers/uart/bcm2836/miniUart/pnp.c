    ...
    )
{
    ...
    {
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "LocalMmMapIoSpace()- using Win10 API\r\n");
         //
    ...
    {
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "LocalMmMapIoSpace()- using std API\r\n");
...

{
    ...
    if (!NT_SUCCESS(status)) {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, " WdfIoQueueCreate for Read failed %!STATUS!\n", status);
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT,  " WdfIoQueueCreate for Write failed %!STATUS!\n", status);
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT,  " WdfIoQueueCreate for Mask failed %!STATUS!\n",   status);
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT,  " WdfIoQueueCreate for Purge failed %!STATUS!\n",   status);
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        ...
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "WdfIoQueueCreate failed %!STATUS!\n", status);
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        ...
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "Couldn't create interrupt for %wZ\n",
                  &pDevExt->DeviceName);
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT,  " WdfWaitLockCreate for InterruptStateLock failed %!STATUS!\n",   status);
        return status;
    }
    ...
    if (!NT_SUCCESS(status)) {
        ...
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "SerialCreateTimersAndDpcs failed %x\n", status);
        return status;
    }
    ...
    if(!NT_SUCCESS (status)) {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "SerialWmiRegistration failed %!STATUS!\n", status);
        return status;
    ...
    if (!NT_SUCCESS(status)) {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "External Naming Failed - Status %!STATUS!\n",
                      status);
        return status;
    }
    ...
    //
    // modify device properties to allow UWP application to access mini Uart device
    // use UART0 name like RhProxy uses until DDA property in UEFI will become available
    //
    //
    status=SerialSetPortNameDevInterfProp(device,L"UART0");
    if(!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "SerialEvtDeviceAdd() ERROR from SerialSetPortNameDevInterfProp\r\n");
    }

    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "--SerialEvtDeviceAdd()=%Xh\r\n",status);
...
NTSTATUS SerialSetPortNameDevInterfProp(WDFDEVICE Device, PCWSTR SerPortName)
{
    ...
    if(!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "SerialSetPortNameDevInterfProp(ERR) %Xh from WdfStringCreate", status);
        goto DevPropEnd2;
    }
    ...
    if(!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "SerialSetPortNameDevInterfProp(ERR) %Xh from WdfDeviceRetrieveDeviceInterfaceString", status);
        goto DevPropEnd;
    }
    ...
    // Note: this is in addition to allowing user mode application to access mini Uart device, which is done via SDDL reg key

    status = IoSetDeviceInterfacePropertyData(&symlinkName,
            &DEVPKEY_DeviceInterface_Serial_PortName,   // Property Key
            LOCALE_NEUTRAL,
            0,                                          // Flags argument seems to be ignored in WDK10
            DEVPROP_TYPE_STRING,                        // prop Type
            (ULONG)(sizeof(wchar_t)*(wcslen(SerPortName)+1)),  // prop size must include null terminator
            (PVOID)SerPortName);                               // Data

    if(!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "SerialSetPortNameDevInterfProp(ERR) %Xh from IoSetDevicePropertyData1", status);
    }

    // need to explicitly set property [IsRestricted]=false to allow UWP application gain access to mini Uart driver
    ...
    status = IoSetDeviceInterfacePropertyData(&symlinkName,
            ...
            LOCALE_NEUTRAL,
            0,
            DEVPROP_TYPE_BOOLEAN,       // prop Type now is boolean
            (ULONG)sizeof(isRestricted),
            &isRestricted);

    if(!NT_SUCCESS(status))
    {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "SerialSetPortNameDevInterfProp(ERR) %Xh from IoSetDevicePropertyData2", status);
    }
...
DevPropEnd2:
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "--SerialSetPortNameDevInterfProp()=%Xh", status);
    return status;
}
...
--*/
{
    ...
    TraceEvents (TRACE_LEVEL_INFORMATION, DBG_PNP, "++SerialEvtPrepareHardware\r\n");
    //
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_PNP, "Com Port ClockRate: %lu Hz (%Xh)\r\n",
    ...
    if(!NT_SUCCESS(status)) 
    {
       TraceEvents (TRACE_LEVEL_ERROR, DBG_PNP, "SerialInitController Failed! Err=%Xh", status);
    }
    else
    {
// print miniuart registers
        bPrintMiniUartregs(pDevExt);
    };
End:

   TraceEvents (TRACE_LEVEL_INFORMATION, DBG_PNP, "<-- SerialEvtPrepareHardware 0x%x\n", status);
...
--*/
{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_PNP, "++SerialEvtDeviceD0EntryPostInterruptsEnabled\r\n");
    //
...
--*/
{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_PNP, "++SerialEvtDeviceD0ExitPreInterruptsDisabled\r\n");
    ...
    )
{
    ...
    if(DeviceExtension->OwnsPowerPolicy && !DeviceExtension->RetainPowerOnClose) {
        ...
        //
        idleSettings.Enabled=WdfFalse; // sr ported from rpi2 other serial driver
...
--*/
{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_POWER, "++SerialReportMaxBaudRate(bauds=%Xh)\r\n",Bauds);

    for(i=0; SupportedBaudRates[i].BaudRate != SERIAL_BAUD_INVALID; i++) 
    {

        if(Bauds & SupportedBaudRates[i].Mask) // as 600, SERIAL_BAUD_600 mask   
        {
            uRetVal=SupportedBaudRates[i].BaudRate;
        }
    }

    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_POWER, "--SerialReportMaxBaudRate()=%lu\r\n",uRetVal);
    return uRetVal;
}
...

{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "++SerialInitController for %wZ\n",
                                                    &pDevExt->DeviceName);
    //
    ...
    if (!pDevExt->Controller) {
      ...
      TraceEvents(TRACE_LEVEL_WARNING, DBG_INIT, "Could not map memory for device "
                       "registers for %wZ\n", &pDevExt->DeviceName);
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "RxFifoTrigger=%u\n",pDevExt->RxFifoTrigger);
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "TxFifoAmount=%u\n",pDevExt->TxFifoAmount);
    ...
    {
        WRITE_MINIUART_ENABLE(pDevExt, pDevExt->Controller, MINIUART_ENABLE_MASK);
    //
    // allow pi firmware some time to perform enabling of mini uart hardware
    //
        liTimeout.QuadPart = (LONGLONG)10*IDLETIMEµs * (LONGLONG)(-10); // convert µs into 100-nanosec units
        ...
        for(i=0;i<ciMaxIter;i++)
        {
            ...
            if(READ_MINIUART_ENABLE(pDevExt,pDevExt->Controller) & MINIUART_ENABLE_MASK)
            {
                TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "Mini UART interface was disabled, now enabled succesfully\r\n");
                break;
            };
        };
    ...
    // next we enable both receiver and transmitter parts of miniuart
    WRITE_MINIUARTRXTX_ENABLE(pDevExt, pDevExt->Controller, 0x3);    
    if((READ_MINIUARTRXTX_ENABLE(pDevExt, pDevExt->Controller) & 0x3) == 0x0)
    {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "Mini UART receive and transmit parts NOT enabled!\r\n");
        status = STATUS_NO_SUCH_DEVICE;
        goto ExtensionCleanup;
    };


    if(FALSE==SerialDoesPortExist(
                           pDevExt,
                           &pDevExt->DeviceName,
      ...
      TraceEvents(TRACE_LEVEL_WARNING, DBG_INIT, "DoesPortExist() DLAB presence test failed for "
                       "%wZ\n", &pDevExt->DeviceName);
    ...
    if (PConfigData->DisablePort) {
      ...
      TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "disabled port %wZ as requested in "
                       "configuration\n", &pDevExt->DeviceName);
    ...
    pDevExt->HandFlow.ControlHandShake = 0; // was SERIAL_DTR_CONTROL - verify todo
    pDevExt->HandFlow.FlowReplace      = 0; // was SERIAL_RTS_CONTROL; // todo
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, " The default interrupt read buffer size is: %d\r\n"
                    "------  The XoffLimit is                         : %d\r\n"
                    "------  The XonLimit is                          : %d\r\n"
                    "------  The pt 8 size is                         : %d\r\n",
    ...
    for(i=0; SupportedBaudRates[i].BaudRate != SERIAL_BAUD_INVALID; i++) 
    {
        ...
        if(!NT_ERROR(status))
        {
            ...
            TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, " %lu Ok. ", SupportedBaudRates[i].BaudRate);
        }
        else
        {
            TraceEvents(TRACE_LEVEL_WARNING, DBG_INIT, " %lu - no. ", SupportedBaudRates[i].BaudRate);
        };
...
#pragma warning(suppress: __WARNING_IRQ_SET_TOO_HIGH; suppress: __WARNING_INVALID_PARAM_VALUE_1) 
    ...
    pDevExt->WmiCommData.SupportRTSCTS = FALSE; // miniUart has it in hardware, bit wired outside, thus not usable
    ...
    pDevExt->WmiCommData.SettableFlowControl = FALSE; // no miniUart flow control pins are wired outside 
...
ExtensionCleanup: ;
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "<-- SerialInitController %x\n", status);
...
// ARM build cannot have io ports
            ASSERT(FALSE);
         break;
        ...
        if ((gotMem == 0) && (gotIO == 0)
                         && (pPartialTrResourceDesc->u.Memory.Length // from ACPI, was 70h
                         >= SERIAL_REGISTER_SPAN )) { // original looks for 7 dwords exactly, needs to be 28h
            gotMem = 1;
            PConfig->TrController = pPartialTrResourceDesc->u.Memory.Start;

            if (!PConfig->TrController.LowPart) {
                TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "Bogus I/O memory address %x\n",
        ...
        case CmResourceTypeInterrupt:
            if (gotInt == 0) {
                ...
                if (!PConfig->TrVector) {
                    TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "Bogus vector 0\n");
                    status = STATUS_DEVICE_CONFIGURATION_ERROR;
                    goto End;
                }

               if (pPartialTrResourceDesc->ShareDisposition == CmResourceShareShared) {
                    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "Sharing interrupt with other devices \n");
                } else {
                    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "Interrupt is not shared with other devices\n");
                }
   ...
   if (DebugPortInUse) {
      ...
      TraceEvents(TRACE_LEVEL_ERROR, DBG_INIT, "Kernel debugger is using port at "
                       "address %p\n", *KdComPortInUse);
...
End:
   ...
   TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "<-- SerialMapHWResources %x\n", status);
...
//        return STATUS_SUCCESS;
    ...
    status = SerialReadSymName(PDevExt->WdfDevice, pRegName, &nameSize); // fails on Windows 10 IoT Core, needs desktop
    ...
    if (!NT_SUCCESS(status) && PDevExt->SkipNaming==0) {
      ...
      TraceEvents(TRACE_LEVEL_ERROR, DBG_PNP, "Couldn't create the device map entry\n"
                       "------- for port %ws\n", PDevExt->DeviceName.Buffer);
    ...
    if (!NT_SUCCESS (status)) {
        TraceEvents(TRACE_LEVEL_ERROR, DBG_PNP, "Couldn't register class association\n"
                       "for port %wZ\n", &PDevExt->DeviceName);
...

{
   ...
   if (value1 == value2) { // note: this check may fail on pi miniuart
...
//   AllDone: ;
   ...
   if (returnValue) {
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_PNP, "--SerialDoesPortExist()=%Xh\r\n",returnValue);
...

{
   ...
   if (extension->FifoPresent) 
   {
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_PNP, "SerialReset() new super IO chip workaround");
      //
      // There is a fine new "super" IO chip out there that
      // will get stuck with a line status interrupt if you
      // attempt to clear the fifo and enable it at the same
   ...
   //
   TraceEvents(TRACE_LEVEL_INFORMATION, DBG_PNP, "SerialReset() set the line control, modem control, and the baud");
   //
   ...
   //
   // NOTE: This does not actually let interrupts
   // occur.  We must still raise the OUT2 bit in the
   // modem control register.  We will do that on open. 
   //
   // Pi platform MiniUart NOTE: out2 does not exist on miniUart
   // interrupts will start firing immediately
   //
//   TraceEvents(TRACE_LEVEL_INFORMATION, DBG_PNP, "SerialReset() Enable ALL interrupts.");
...
BOOLEAN bPrintMiniUartregs(PSERIAL_DEVICE_EXTENSION pDevExt)
{
    ...
    {
        uchTempReg=READ_MINIUART_ENABLE(pDevExt, pDevExt->Controller); // 0x3E215004 Enable
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "Enabled=%Xh\r\n",uchTempReg);
        uchTempReg=READ_RECEIVE_BUFFER(pDevExt, pDevExt->Controller); // 0x3E215040 THR/RCV
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "THR/RCV=%Xh\r\n",uchTempReg);
        uchTempReg=READ_INTERRUPT_ENABLE(pDevExt, pDevExt->Controller); // 0x3E215044 IER
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "IER=%Xh\r\n",uchTempReg);
        uchTempReg=READ_INTERRUPT_ID_REG(pDevExt, pDevExt->Controller); // 0x3E215048 IIR
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "IIR=%Xh\r\n",uchTempReg);
        uchTempReg=READ_LINE_CONTROL(pDevExt, pDevExt->Controller); // 0x3E21504C LCR
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "LCR=%Xh\r\n",uchTempReg);
        uchTempReg=READ_MODEM_CONTROL(pDevExt, pDevExt->Controller); // 0x3E215050 MCR
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "MCR=%Xh\r\n",uchTempReg);
        uchTempReg=READ_LINE_STATUS(pDevExt, pDevExt->Controller); // 0x3E215054 LSR
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "LSR=%Xh\r\n",uchTempReg);
        uchTempReg=READ_MODEM_STATUS(pDevExt, pDevExt->Controller); // 0x3E215058 MSR
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "MSR=%Xh\r\n",uchTempReg);
        uchTempReg=READ_MINIUARTRXTX_ENABLE(pDevExt, pDevExt->Controller); // 0x3E215060 extra control
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "ExtraCTRL=%Xh\r\n",uchTempReg);
        uchTempReg32=READ_EXTRA_STATUS(pDevExt->Controller); // 0x3E215064 extra status, 32-bit
        TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "ExtraSTS=%08Xh\r\n",uchTempReg32);
        uchTempReg16=READ_EXTRA_BAUD(pDevExt->Controller); // 0x3E215068 extra status, 16-bit
