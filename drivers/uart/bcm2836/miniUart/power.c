...
#pragma prefast(suppress: __WARNING_INFERRED_IRQ_TOO_LOW, "PFD warning that we are calling interrupt synchronize routine directly. Suppress it because interrupt is disabled above.")
    ...
    if (deviceExtension->DeviceState.Reopen == TRUE) {
       TraceEvents(TRACE_LEVEL_INFORMATION, DBG_POWER, "Reopening device\n");
...
--*/
{
    ...
    TraceEvents(TRACE_LEVEL_INFORMATION, DBG_POWER, "++SerialEvtDeviceD0Exit - moving to %s\r\n", DbgDevicePowerString(TargetState));
...
--*/
{
   ...
   TraceEvents(TRACE_LEVEL_INFORMATION, DBG_POWER, "Entering SerialSaveDeviceState\n");
   ...
   TraceEvents(TRACE_LEVEL_INFORMATION, DBG_POWER, "Leaving SerialSaveDeviceState\n");
}
