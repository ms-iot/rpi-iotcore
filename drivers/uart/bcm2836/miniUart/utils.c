...

{
   ...
   ULONG maxRemain = ClockRate/100;
    ...
    TraceEvents(TRACE_LEVEL_VERBOSE, DBG_INIT, "++SerialGetDivisorFromBaud(clock=%lu, desiredbaudr=%lu)\r\n",ClockRate,DesiredBaud);
/*
 Note: the following formula for baud rate divisor on miniUart on RPi3 is different from normal 16550
 MU_BAUD_REG is 16 bit register which holds clock divisor. The same divisor value can be written in two parts into normal THR(LS), IER(MS) registers
...
*/
   ...
   else 
   {
      ...
      TraceEvents(TRACE_LEVEL_VERBOSE, DBG_INIT, "SerialGetDivisorFromBaud() disv=%d, remndr=%lu\r\n",calculatedDivisor,remainder);
      ...
      if (DesiredBaud >= 9600) 
      {
         ...
         {
            TraceEvents(TRACE_LEVEL_WARNING, DBG_INIT, "SerialGetDivisorFromBaud(baud=%ld) divisor=%d out of range \r\n",DesiredBaud,calculatedDivisor);
      ...
      if(denominator > ClockRate)
      {
         calculatedDivisor = (USHORT)-1;
         *AppropriateDivisor=-1;
         TraceEvents(TRACE_LEVEL_WARNING, DBG_INIT, "SerialGetDivisorFromBaud(baud=%ld) denominator > ClockRate\r\n",DesiredBaud);
      };

      *AppropriateDivisor = calculatedDivisor;
   };

   TraceEvents(TRACE_LEVEL_VERBOSE, DBG_INIT, "SerialGetDivisorFromBaud(clock=%lu, desiredbaudr=%lu) apprdivisor=%d \r\n",ClockRate,DesiredBaud,*AppropriateDivisor);
   ...
   else
   {
       ...
       TraceEvents(TRACE_LEVEL_INFORMATION, DBG_INIT, "SerialGetDivisorFromBaud() desired baudr=%lu, actual baudr=%lu, deviation=%ld.%u%%\r\n",
        DesiredBaud, ulAtualBaudRate, lBaudRateDevPercBy10/10, abs(lBaudRateDevPercBy10%10));
   };

   TraceEvents(TRACE_LEVEL_VERBOSE, DBG_INIT, "--SerialGetDivisorFromBaud(clock=%lu, desiredbaudr=%lu)=%Xh\r\n",ClockRate,DesiredBaud,status);
